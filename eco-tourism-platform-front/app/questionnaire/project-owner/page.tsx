...
const CATEGORY_LABELS: Record<string, string> = {
  ...
  economic: "Économique Local",
};

const OWNER_PROFILES: Record<string, string> = {
  "Ambassadeur durable": "Prestataire Ambassadeur",
  "Écovoyageur engagé": "Prestataire Engagé",
  "Voyageur sensible": "Prestataire Sensible",
  "Voyageur classique": "Prestataire en Développement",
};

const OWNER_MESSAGES: Record<string, string> = {
  "Ambassadeur durable": "Félicitations ! Votre projet est un modèle d'éco-tourisme responsable en Tunisie.",
  "Écovoyageur engagé": "Excellent ! Votre projet affiche un très haut niveau de responsabilité durable.",
  "Voyageur sensible": "Bien ! Votre projet intègre de bonnes pratiques éco-responsables.",
  "Voyageur classique": "La plateforme Éco-Voyage vous accompagnera pour renforcer la durabilité de votre projet.",
};
...
function ScoreCard({ result }: { result: ScoreResult }) {
  ...
  const ownerProfile = OWNER_PROFILES[result.profile] ?? result.profile;
  const ownerMessage = OWNER_MESSAGES[result.profile] ?? result.message;
    ...
    <div className="flex flex-col items-center text-center">
      <div className="relative mb-8">
        <div className="w-44 h-44 rounded-full border-8 border-primary/20 flex flex-col items-center justify-center bg-white shadow-xl">
          ...
          <span className="text-xs text-slate-400 font-bold uppercase tracking-wider mt-1">Score projet</span>
        </div>
        <div className="absolute -top-2 -right-2 w-12 h-12 rounded-full bg-primary flex items-center justify-center shadow-lg">
          <span className="material-symbols-outlined text-slate-900 text-xl">domain</span>
        </div>
      </div>

      <h2 className="text-2xl font-extrabold text-slate-900 mb-1">{ownerProfile}</h2>
      <p className="text-slate-500 max-w-md mb-8 font-medium">{ownerMessage}</p>

      <div className="w-full max-w-md space-y-4 mb-8">
        {[
          { label: "Impact environnemental", value: result.environmental_score, color: "bg-green-500" },
          { label: "Impact social & communautaire", value: result.social_score, color: "bg-blue-500" },
          { label: "Ancrage économique local", value: result.economic_score, color: "bg-orange-400" },
        ].map((sub) => (
          <div key={sub.label}>
      ...
      <div className="w-full max-w-md p-5 bg-primary/10 rounded-2xl border border-primary/20 mb-8 text-left">
        ...
        <ul className="text-sm text-slate-600 space-y-1 font-medium">
          <li>• Valoriser votre projet auprès des voyageurs éco-responsables</li>
          <li>• Vous proposer des certifications et labels adaptés</li>
          <li>• Suivre l'impact durable de votre établissement</li>
        </ul>
      </div>
...
export default function ProjectOwnerQuestionnairePage() {
  const router = useRouter();
      ...
      <div className="min-h-screen bg-background flex flex-col">
        <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 px-6 py-4">
          <div className="max-w-2xl mx-auto flex items-center gap-2">
            ...
            <span className="text-lg font-extrabold tracking-tight">Évaluation Projet</span>
          </div>
        </header>
              ...
              {currentQuestion === 0 && answeredCount === 0 && (
                <div className="bg-primary/10 border border-primary/20 rounded-2xl p-5 mb-6">
                  <p className="text-sm font-medium text-slate-700 leading-relaxed">
                    <strong>🏡 En tant que propriétaire</strong>, répondez aux situations suivantes en choisissant ce qui correspond le mieux aux pratiques actuelles de votre projet touristique.
                  </p>
                </div>
              )}
